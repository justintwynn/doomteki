    ...
    render() {
        return (
            <div className={ this.props.active ? 'deck-row active' : 'deck-row' } key={ this.props.deck.name } onClick={ this.props.onClick }>
                <img className='pull-left' src={ '/img/cards/' + this.props.deck.faction.value + '.png' } />
                <div>{ this.props.deck.name }<span className='pull-right'>{ this.props.deck.validation.status === 'Valid' ? 'Valid' : 'Invalid' }</span></div>
